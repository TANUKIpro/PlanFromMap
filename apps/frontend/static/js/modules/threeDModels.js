 ...
 *
 * 改善: 共通ジオメトリモジュールを使用してコードを簡素化
 *
 * @requires ../utils/threeDUtils.js - 3D描画ユーティリティ
 * @requires ./threeDGeometry.js - 共通ジオメトリ描画
 *
...
export function draw3DShelf(ctx, coords3D, color, centerX, centerY, frontDirection, objectProperties, worldToIso, state) {
    ...
    const geometry = createShelfGeometry(coords3D, worldToIso, centerX, centerY, state, {
        frontDirection,
        color,
        shelfLevels: objectProperties?.shelfLevels
    });
    ...
    // 棚板を描画
    if (geometry.shelves) {
        drawShelfShelves(ctx, geometry, color);
    }
...
export function draw3DTable(ctx, coords3D, color, centerX, centerY, worldToIso, state) {
    ...
    const geometry = createTableGeometry(coords3D, worldToIso, centerX, centerY, state, { color });
    drawBoxFaces(ctx, geometry, state);
