 ...
 * PGMデータを指定範囲でクロップ
 *
 * この関数は、PGMフォーマットのピクセルデータを指定された矩形範囲で切り出します。
 * 元の画像データから必要な領域のみを抽出し、新しいPGMデータオブジェクトを生成します。
 *
 * @param {Object} pgmData - 元のPGMデータ
 * @param {number} pgmData.width - 元の幅
 * @param {number} pgmData.height - 元の高さ
 * @param {number} pgmData.maxVal - 最大値
 * @param {Uint8Array} pgmData.data - ピクセルデータ
 * @param {number} minX - クロップ範囲の最小X座標
 * @param {number} minY - クロップ範囲の最小Y座標
 * @param {number} maxX - クロップ範囲の最大X座標
 * @param {number} maxY - クロップ範囲の最大Y座標
 * @returns {Object} クロップされたPGMデータ
 * @returns {number} returns.width - クロップ後の幅
 * @returns {number} returns.height - クロップ後の高さ
 * @returns {number} returns.maxVal - 最大値（元のまま）
 * @returns {Uint8Array} returns.data - クロップされたピクセルデータ
 * @returns {number} returns.offsetX - 元画像からのXオフセット
 * @returns {number} returns.offsetY - 元画像からのYオフセット
 *
 * @example
 * const pgmData = parsePGM(uint8Array);
 * const bounds = { minX: 10, minY: 20, maxX: 100, maxY: 120 };
 * const cropped = cropPGMData(pgmData, bounds.minX, bounds.minY, bounds.maxX, bounds.maxY);
 */
export function cropPGMData(pgmData, minX, minY, maxX, maxY) {
    // 境界チェック
    minX = Math.max(0, Math.floor(minX));
    minY = Math.max(0, Math.floor(minY));
    maxX = Math.min(pgmData.width - 1, Math.floor(maxX));
    maxY = Math.min(pgmData.height - 1, Math.floor(maxY));

    const newWidth = maxX - minX + 1;
    const newHeight = maxY - minY + 1;

    if (newWidth <= 0 || newHeight <= 0) {
        console.error('cropPGMData: 無効なクロップ範囲', { minX, minY, maxX, maxY });
        return pgmData; // 元のデータを返す
    }
    ...
    for (let y = 0; y < newHeight; y++) {
        for (let x = 0; x < newWidth; x++) {
            const srcX = minX + x;
            const srcY = minY + y;
            const srcIndex = srcY * pgmData.width + srcX;
            const dstIndex = y * newWidth + x;
            newData[dstIndex] = pgmData.data[srcIndex];
        }
    }

    const originalSize = pgmData.width * pgmData.height;
    const croppedSize = newWidth * newHeight;
    const reductionPercent = ((1 - croppedSize / originalSize) * 100).toFixed(1);

    console.log(`cropPGMData: ${pgmData.width}x${pgmData.height} → ${newWidth}x${newHeight} (${reductionPercent}% 削減)`);
...
/**
 * PGMデータから有効領域（黒+白ピクセル）を抽出してクロップ
 *
 * この関数は、PGMデータの有効領域を自動検出し、その範囲でクロップします。
 * 黒色ピクセル（障害物、≤blackThreshold）と白色ピクセル（自由空間、≥whiteThreshold）
 * のみを有効領域とし、グレー（未知領域）は除外します。
 *
 * 用途：
 * - PGMファイルのテクスチャサイズ最適化
 * - メモリ使用量の削減
 ...
 *
 * @param {Object} pgmData - 元のPGMデータ
 * @param {number} [blackThreshold=100] - 黒とみなす輝度値の上限
 * @param {number} [whiteThreshold=220] - 白とみなす輝度値の下限
 * @param {number} [marginPercent=0.05] - 追加マージン（デフォルト5%）
 * @returns {Object|null} クロップされたPGMデータとメタ情報、失敗時はnull
 * @returns {Object} returns.pgmData - クロップされたPGMデータ
 * @returns {Object} returns.bounds - 検出された境界情報
 *
 * @example
 * const pgmData = parsePGM(uint8Array);
 * const result = extractValidRegion(pgmData);
 * if (result) {
 *     const img = pgmToImage(result.pgmData);
 * }
 */
export function extractValidRegion(pgmData, blackThreshold = 100, whiteThreshold = 220, marginPercent = 0.05) {
    if (!pgmData || !pgmData.data) {
        console.error('extractValidRegion: 無効なPGMデータ');
        return null;
    }
    ...
    // 有効ピクセルが見つからない場合は元のデータを返す
    if (validRows.size === 0 || validCols.size === 0) {
        console.warn('extractValidRegion: 有効な領域が見つかりませんでした。元の画像を使用します。');
        return {
            pgmData: pgmData,
            bounds: {
                minX: 0,
    ...
    const marginX = Math.ceil(cropWidth * marginPercent);
    const marginY = Math.ceil(cropHeight * marginPercent);

    const finalMinX = Math.max(0, minX - marginX);
    const finalMinY = Math.max(0, minY - marginY);
    const finalMaxX = Math.min(width - 1, maxX + marginX);
    const finalMaxY = Math.min(height - 1, maxY + marginY);

    const validPixelCount = validRows.size * validCols.size;
    const totalPixels = width * height;
    const validPercent = ((validPixelCount / totalPixels) * 100).toFixed(1);

    console.log(`extractValidRegion: 有効領域検出 [${finalMinX}, ${finalMinY}] - [${finalMaxX}, ${finalMaxY}] (有効ピクセル: ${validPercent}%)`);

    // PGMデータをクロップ
    const croppedPGM = cropPGMData(pgmData, finalMinX, finalMinY, finalMaxX, finalMaxY);
